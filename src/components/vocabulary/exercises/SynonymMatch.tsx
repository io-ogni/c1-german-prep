...
/** Color palette for matched pairs — avoids red/green (used for correct/incorrect) */
const PAIR_COLORS = [
...
export function SynonymMatch({ content, solution, instructions, explanation, answered, onAnswer }: Props) {
  ...
  const handleCheck = () => {
    ...
    for (let i = 0; i < shuffledLeft.length; i++) {
      ...
      if (rightIdx === undefined || !shuffledRight[rightIdx] || !shuffledLeft[i] || shuffledRight[rightIdx].originalIdx !== shuffledLeft[i].originalIdx) {
        allCorrect = false;
        break;
      }
    }
  ...
  const isMatchCorrect = (leftIdx: number): boolean | null => {
    ...
    if (rightIdx === undefined || !shuffledRight[rightIdx] || !shuffledLeft[leftIdx]) return false;
    return shuffledRight[rightIdx].originalIdx === shuffledLeft[leftIdx].originalIdx;
  };

  // Build a map from right index to the color of its matched left item
