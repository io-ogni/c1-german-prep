...
export const NAV_CONTAINER = 'inline-flex items-center flex-wrap bg-muted/80 border border-border p-1 gap-0.5 rounded-lg';

/** Tab trigger for blue secondary nav — Radix TabsTrigger */
export const TAB_TRIGGER_BLUE =
  'data-[state=inactive]:text-foreground/70 data-[state=inactive]:hover:bg-primary/10 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground';
...
export const navFuchsiaClasses = (isActive: boolean) =>
  isActive
    ...
    : 'text-foreground/70 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-900/20';

/* ── Tertiary nav (pill style) ─────────────────────────────────── */

/** Container: transparent background, flex-wrap with gap between pills */
export const PILL_CONTAINER = 'flex flex-wrap h-auto gap-1.5 justify-start bg-transparent p-0';

/** Pill trigger for fuchsia tertiary nav — Radix TabsTrigger (ITVokabularPage) */
export const TAB_TRIGGER_FUCHSIA =
  'gap-1.5 rounded-xl border border-border px-3 py-1.5 text-xs shadow-sm data-[state=inactive]:text-foreground/70 data-[state=inactive]:hover:bg-fuchsia-50 dark:data-[state=inactive]:hover:bg-fuchsia-900/20 data-[state=active]:border-fuchsia-500 data-[state=active]:bg-fuchsia-500 data-[state=active]:text-white';
...
export const pillBlueClasses = (isActive: boolean) =>
  isActive
    ...
    : 'border border-border shadow-sm text-foreground/70 hover:bg-primary/10';
...
export const pillFuchsiaClasses = (isActive: boolean) =>
  isActive
    ...
    : 'border border-border shadow-sm text-foreground/70 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-900/20';
